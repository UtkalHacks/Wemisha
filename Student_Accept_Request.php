	...
	<div class="backimg">
       ...
       <div class="main-section">
        ...
        <?php
          ...
          $query="select * from request_details where reciever='$roll_no' and request='0'";
          $result=mysqli_query($con,$query);
          if($result)
            { 
               $row_count=mysqli_num_rows($result);
       ...
       if($request_id=="")
         {
             ...
             mysqli_close($con);
             echo '<script type="text/javascript"> location.href="./Student_Accept_Request.php"</script>';
         }
        
        $query="select * from request_details where sender='$request_id' and reciever='$roll_no'";
        $result=mysqli_query($con,$query);
        if($result)
          { 
            $row_count=mysqli_num_rows($result);
            if($row_count==1)
              { 
                 ...
                 if($request=="1")
                  ...
                  { 
                      $query1="update request_details set request='1' where sender='$request_id' and reciever='$roll_no'";
...
<?php
   if(isset($_POST['btn_accept_all']))
    { 
      $reciever=$roll_no;
      $query="update request_details set request='1' where reciever='$roll_no' and request='0'";
      $result=mysqli_query($con,$query);

      if($result)
      {
             echo '<script type="text/javascript"> alert("You have accepted all of your Chat Requests.") </script>';
             mysqli_close($con);
             echo '<script type="text/javascript"> location.href="./Student_Accept_Request.php"</script>';
      }
    }
