...
<?php
     ...
     { 
      ...
      if(($p_text=="" and $file_tmp==""))
        { 
           ...
           mysqli_close($con);
           echo '<script type="text/javascript"> location.href="./Student_Settings.php"</script>';
        }
      else
        { 
  ...
  if(isset($_POST['btn_change']))
  {
    ...
    if($id=="" || $c_new_id==""|| $c_new_id=="")
      ...
      { 
         $query="select * from student_details where roll_no='$roll_no' and password='$id'";
         $result=mysqli_query($con,$query); 
         if($result)
         {
          $row_count=mysqli_num_rows($result);
         ...
         $query="update student_details set password='$new_id' where roll_no='$roll_no'";
         $result=mysqli_query($con,$query);
         if($result)
            {
             session_destroy();
