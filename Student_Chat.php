       ...
       <div class="main-section">
        ...
        <div class="request-tab">
         ...
         <div class="chat-request">
          ...
          $query="select * from request_details where (sender='$roll_no' or reciever ='$roll_no') and request='1'";
          $result=mysqli_query($con,$query);
          if($result)
            { 
               $row_count=mysqli_num_rows($result);
                ...
                { 
                   while($row=mysqli_fetch_assoc($result))
                    { 
                           ...
                           if($row['sender']==$roll_no)
                              $id=$row['reciever'];
                           else if($row['reciever']==$roll_no)
                              $id=$row['sender'];
  ...
  if(isset($_POST['btn_open']))
  {
   ...
   else
    {
        ...
        if($result)
         ...
         $query="select * from request_details where sender='$roll_fid' and reciever ='$roll_no' or reciever ='$roll_fid' and sender ='$roll_no' ";
         $result=mysqli_query($con,$query);
         if($result)
          { 
            $row=mysqli_fetch_assoc($result);
            if($row['request']=="0")
              ...
              { 
                ...
                mysqli_close($con);
                echo '<script type="text/javascript"> location.href="./Student_Open_Chat.php"</script>';
              }
          }
    }
  }
