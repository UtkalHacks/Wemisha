...
<html>
  ...
  <head>
    <title>FRIENDZY | HOD Approve Request</title>
    <style type="text/css">
	...
	<div class="backimg">
       ...
       <div class="main-section">
         <div class="request-tab">
            <nav>
              <ul class="tab-items">
                 <li><a class="tab-fix" href="#">Approve Student</a></li>
                 <li><a class="tab-hov" href="./HOD_Approve_Post.php">Approve Faculty</a></li>
              </ul>
            </nav>
         </div>
         ...
         <?php
          ...
          $query="select f_name,m_name,l_name,roll_no from student_details where reg_status=0";
          $result=mysqli_query($con,$query);
          if($result)
           {
              $row_count=mysqli_num_rows($result);
              if($row_count>0)
                {
                  while($row=mysqli_fetch_assoc($result))
                  {
                    ...
                    if($roll_id==$_SESSION['department_id'])
                    {
                      echo "<div class='request'> 
                              <form>
                                 <table style='color:white; font-size:18px; font-family:Bookman Old Style;'>
                                    <tr>
                                      <td style='padding-left:60px; padding-top:15px;'>$roll_no</td>
       ...
       <div style="background-color:rgba(0,0,0,0.6); height:70px; width:725px; margin-top:390px">
          <form method="post">
             <table>
                <tr>
                   <td><input class="textbox" type="text" name="txt_approve" placeholder="Enter roll no "></td>
  ...
  <?php
          ...
          {
            $approval_roll_no=test_input($_POST['txt_approve']);
            
            if($approval_roll_no=="")
              { 
                echo '<script type="text/javascript"> alert("Please enter the Roll No.") </script>';
                mysqli_close($con);
                echo '<script type="text/javascript"> location.href="./HOD_Approve_Request.php"</script>';
              }

            else
            {
              
               $query="update student_details set reg_status=1 where roll_no='$approval_roll_no'";
               $result=mysqli_query($con,$query);
            
               if($result)
                   echo '<script type="text/javascript"> alert("Student Approved.") </script>';
            
               else
                   echo '<script type="text/javascript"> alert("Roll No does not exist.") </script>';
       ...
       <?php
          ...
          {
            $query="update student_details set reg_status=1 where 1";
            $result=mysqli_query($con,$query);
            
               if($result)
                   echo '<script type="text/javascript"> alert("All Students have been Approved.") </script>';
            
               else
                   echo '<script type="text/javascript"> alert("Students already approved.") </script>';
